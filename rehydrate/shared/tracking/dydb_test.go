...
func TestDyDBStore_NewInProgressEntry(t *testing.T) {
	ctx := context.Background()
	awsConfig := test.NewAWSEndpoints(t).WithDynamoDB().Config(ctx, false)
	store := tracking.NewStore(awsConfig, logging.Default, testTableName)
	...
	expectedFargateTaskARN := "arn:ecs:test::test"
	beforeRequest := time.Now()
	err := store.NewInProgressEntry(ctx, expectedID, dataset, user, expectedLambdaLog, expectedAWSRequestID, expectedFargateTaskARN)
	require.NoError(t, err)
	...
	AssertEqualAttributeValueString(t, expectedFargateTaskARN, actualItem[tracking.FargateTaskARNAttrName])

	AssertEqualAttributeValueString(t, string(tracking.InProgress), actualItem[tracking.RehydrationStatusAttrName])
...
func TestDyDBStore_NewFailedEntry(t *testing.T) {
	ctx := context.Background()
	awsConfig := test.NewAWSEndpoints(t).WithDynamoDB().Config(ctx, false)
	store := tracking.NewStore(awsConfig, logging.Default, testTableName)
	...
	expectedAWSRequestID := "REQUEST-8765"
	beforeRequest := time.Now()
	err := store.NewFailedEntry(ctx, expectedID, dataset, user, expectedLambdaLog, expectedAWSRequestID)
	require.NoError(t, err)
	afterRequest := time.Now()
	...
	AssertEqualAttributeValueString(t, string(tracking.Failed), actualItem[tracking.RehydrationStatusAttrName])
	var requestDate time.Time
	if assert.NoError(t, attributevalue.Unmarshal(actualItem[tracking.RequestDateAttrName], &requestDate)) {
		assert.True(t, beforeRequest.Equal(requestDate) || beforeRequest.Before(requestDate))
		assert.True(t, afterRequest.Equal(requestDate) || afterRequest.After(requestDate))
